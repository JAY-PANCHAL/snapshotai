...
export default function PromptBuilder({ selectedTemplate, photoData, prefillPrompt, onPromptChange }) {
  ...
  }, [template, background, lighting, attire, mood, platform, quality, extras, customNotes, onPromptChange]);

  // Build prompt on settings change (fires first on mount, before prefill effect)
  useEffect(() => {
    buildFinalPrompt();
  }, [buildFinalPrompt]);

  // Prefill from sample gallery / external source — runs AFTER buildFinalPrompt so it wins on mount
  useEffect(() => {
  ...
  const handleAiEnhance = async () => {
    setAiEnhancing(true);
    toast('Enhancing prompt with AI…', 'info', 5000);
    ...
    if (enhanced) {
      ...
      toast('Enhancement failed — API key required', 'error');
    }
